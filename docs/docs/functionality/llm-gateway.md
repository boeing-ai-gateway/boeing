...
The Boeing LLM Gateway lets you call OpenAI and Anthropic models through Boeing using an **Boeing API key** instead of a provider API key. Point any OpenAI- or Anthropic-compatible client — such as [Claude Code](#using-with-claude-code) or [Codex](#using-with-codex) — at the gateway, authenticate with an API key that has LLM proxy access, and call models by their native provider names (for example `claude-opus-4.8` or `gpt-5.5`).

The gateway proxies your requests transparently to the upstream provider while enforcing per-user access:

- You never handle the provider's real API key. Boeing holds the key (configured by an administrator on a [Model Provider](/configuration/model-providers/)) and substitutes it on each request.
...
- **Base URL** — the gateway endpoint to point your client at, with a copy button:
  - OpenAI: `https://<your-boeing-host>/api/llm-proxy/openai`
  - Anthropic: `https://<your-boeing-host>/api/llm-proxy/anthropic`
- **Example request** — a ready-to-run `curl` command, pre-filled with one of your available models and with the API key wired to `boeing login --scope llm --print-token`.
...
3. **The Boeing CLI.** Install and set up the `boeing` CLI to obtain an API key. See [Boeing CLI Setup](/installation/cli-setup/).
...
```bash
export ANTHROPIC_BASE_URL="https://boeing.example.com/api/llm-proxy/anthropic"
export ANTHROPIC_API_KEY="$(boeing login --url https://boeing.example.com --scope llm --print-token)"
...
```bash
export OPENAI_BASE_URL="https://boeing.example.com/api/llm-proxy/openai"
export OPENAI_API_KEY="$(boeing login --url https://boeing.example.com --scope llm --print-token)"
...
```bash
# 1. Point Claude Code at the gateway and authenticate with your Boeing API key.
export ANTHROPIC_BASE_URL="https://boeing.example.com/api/llm-proxy/anthropic"
export ANTHROPIC_API_KEY="$(boeing login --url https://boeing.example.com --scope llm --print-token)"
...
- Run **`/model`** and select a Boeing gateway model. Discovered models are labeled **"From gateway"**.
...
`ANTHROPIC_API_KEY` is sent as the `x-api-key` header. If your setup prefers a bearer token, you can set `ANTHROPIC_AUTH_TOKEN` instead (sent as `Authorization: Bearer`). The gateway accepts either header with a Boeing API key that has LLM proxy access.
:::
...
1. Add the following to `~/.codex/config.toml`:
   ...
   ```toml
   model_provider = "boeing_openai"

   [model_providers.boeing_openai]
   name = "OpenAI Boeing LLM Gateway"
   base_url = "https://boeing.example.com/api/llm-proxy/openai"
   env_key = "BOEING_API_KEY"
   env_key_instructions = "Set BOEING_API_KEY and restart to authenticate with the Boeing LLM Gateway"
...
2. Set your API key and start Codex:
   ...
   ```bash
   export BOEING_API_KEY="$(boeing login --url https://boeing.example.com --scope llm --print-token)"
   codex        # Codex CLI
   # or
   codex app    # Codex App
   ```
...
:::note
Codex uses the OpenAI Responses API by default, which is what the gateway serves. The provider ID (`boeing_openai` above) can be any name except the reserved IDs `openai`, `ollama`, and `lmstudio`.
:::
...
- **Claude Code model discovery caveats.** Gateway model discovery is **off by default**, requires **Claude Code v2.1.129+**, and only adds models whose IDs begin with `claude` or `anthropic`. Boeing's Anthropic models qualify; if discovery doesn't surface a model, select it manually with `/model`.
...
- **Usage and policies still apply.** Requests count toward Boeing [token usage](/functionality/audit-logs-and-usage/) and, where configured, are subject to [Message Policies](/functionality/message-policies/).
...
- [Boeing CLI Setup](/installation/cli-setup/) — install and configure the `boeing` CLI
