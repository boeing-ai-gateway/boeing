---
title: Boeing Agent Management
---

# Boeing Agent Management

:::note
Boeing Agent features are disabled by default for new deployments, and the **Boeing Agent Management** section is hidden while they are disabled. To enable them, set the `BOEING_ENABLE_AGENTS=true` environment variable on the server. Deployments that already had agents before upgrading remain enabled automatically. See the [configuration reference](../configuration/server-configuration.md) for details.
:::

Boeing Agent Management provides administrators with tools to configure default agent settings and monitor agent and conversation activity. Access these features from **Boeing Agent Management** in the sidebar.
...
Control which users and groups can access which models in Boeing Agent. See [Model Access Policies](/functionality/model-access-policies/) for details.
