...
If `BOEING_TOKEN` is not set and no valid API key is stored locally, `boeing scan --submit` may use the browser-based login flow. For unattended jobs, set `BOEING_TOKEN` to an API key with API access.

### Server submission fails

Check that the configured Boeing server is reachable from the workstation and that the API key has permission to submit device scans.
