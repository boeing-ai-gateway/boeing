...
:::note
Workflows are an [Boeing Agent](../concepts/boeing-agent.md) feature, which is disabled by default for new deployments. To enable it, set the `BOEING_ENABLE_AGENTS=true` environment variable on the server. See the [configuration reference](../configuration/server-configuration.md) for details.
:::
