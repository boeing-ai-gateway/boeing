...
Boeing follows the Agent Skills standard. See [agentskills.io](https://agentskills.io) for more information about skills.

## Skill Sources

Skill sources are GitHub repositories that contain one or more skills. When you add a source, Boeing scans the repository for directories containing `SKILL.md` files and indexes each valid skill it finds. Sources are synced automatically every hour and can also be refreshed manually.

To manage skill sources, go to **Boeing Agent Management > Skills** and select the **Sources** tab.
...
After saving, Boeing fetches the repository and discovers skills. The sync status appears next to the source entry, showing whether the sync is in progress, how many skills were found, or any errors that occurred.

> Try out `https://github.com/boeing-ai-gateway/skills` to access some examples.
...
To view all discovered skills, go to **Boeing Agent Management > Skills** and select the **Skills** tab.
...
When agents are running in Boeing, they have built-in tools for working with skills:
