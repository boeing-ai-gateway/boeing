...
2. Authenticates to that Boeing server. If `BOEING_TOKEN` is set, the CLI uses that token. Otherwise, it uses the same browser-based token flow as `boeing login`.
3. Stores the normalized default Boeing URL in the local Boeing CLI config.
4. Stores a newly acquired Boeing bearer token in the host OS keyring, scoped to that Boeing URL.
...
- A bearer token to the host OS keyring under the `boeing` service, scoped by Boeing app URL, when setup acquires a new token through the login flow.
