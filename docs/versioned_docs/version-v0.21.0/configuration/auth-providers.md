...
Authentication providers allow your Boeing installation to authenticate users with the identity provider of your choice.
...
:::note
In order for authentication to be enabled, the Boeing server must be run with the environment variable set:
...
When launching Boeing for the first time, the server will print a randomly generated bootstrap token to the console.
...
You can use the `BOEING_BOOTSTRAP_TOKEN` environment variable to provide a specific value for the token,
rather than having one generated for you. If you do this, the value will **not** be printed to the console.

Boeing will persist the value of the bootstrap token on its first launch (whether randomly generated or
supplied by `BOEING_BOOTSTRAP_TOKEN`), and all future server launches will use that same value.
`BOEING_BOOTSTRAP_TOKEN` can always be used to override the stored value.
:::

### Preconfiguring Owner & Admin Users

If you want to preconfigure owner or admin users, you can set the `BOEING_SERVER_AUTH_OWNER_EMAILS` or `BOEING_SERVER_AUTH_ADMIN_EMAILS` environment variable, respectively.
...
Boeing currently supports the following authentication providers (using OAuth2). Before getting started you will need to follow the instructions in the auth provider for setting up a new app. You can get the callback URL from the Boeing Admin -> Auth Providers -> \<Auth Provider> -> Configure page. The configuration form will also have fields for the data required.
...
You can view the source code for GitHub provider in this [repo](https://github.com/boeing-ai-gateway/tools).

### Google

Follow the instructions [here](https://developers.google.com/identity/protocols/oauth2/web-server#creatingcred) to create the OAuth app for Boeing.

You can view the source code for Google provider in this [repo](https://github.com/boeing-ai-gateway/tools).
...
Under Supported account types, ensure `Accounts in this organizational directory only` is selected. In the Redirect URI section, set the platform to `Web` and enter the redirect URI provided in Boeing's Auth Provider configuration dialog.
...
You can now return to Boeing and finish configuring Entra. Use the table below to determine the values to use for each field:

| Boeing          | Entra                   | Entra App Tab          |
|---------------|-------------------------|------------------------|
...
3. Select the application corresponding to your Boeing instance
...
Make sure to add the redirect URL displayed in Boeing to the list of Sign-in redirect URIs in your Okta application settings.

Take note of the Client ID and Client Secret. You will need to provide these to Boeing.
...
or have Okta do it for you. Make sure you save the private key in PEM format, as you will need to provide that to Boeing,
...
1. While creating the `OIDC` App for Boeing, select the `Skip group assignment for now` option in the `Assignments` configuration section
...
4. Select `Assign` on groups you want to allow Boeing access to
...
1. Set the **Redirect URI / Callback URL** to the redirect URI displayed in Boeing's Auth Provider configuration dialog
...
4. Configure the app to include group membership claims:
   - Enable **Include Group Attribute** and set the exact attribute name to `memberOf`. This is required for group-based authorization in Boeing.
...
Boeing uses the JumpCloud management API to:
...
You must provide one of the following in the Boeing Auth Provider configuration:

- a JumpCloud service account client ID and client secret. This is the preferred option for JumpCloud group lookup and assignment in Boeing.
- or an API key with access to read users and user groups

If you use a service account:

1. Create a JumpCloud API service account for Boeing in the JumpCloud admin console
2. Generate or reveal its **Client ID** and **Client Secret**
3. Read-only permissions are sufficient

If you use service-account credentials and your JumpCloud region requires a different admin OAuth endpoint, override the token URL in Boeing as well.

You can now return to Boeing and finish configuring JumpCloud. Use the table below to determine the values to use for each field:
...
1. Set the **Allowed Callback URLs** to the redirect URI displayed in Boeing's Auth Provider configuration dialog
2. Take note of the **Domain**, **Client ID**, and **Client Secret**

:::important Email Verification
Auth0 users must have a verified email address to log in to Boeing. If a user's email is not verified, the login will fail with a 500 error.
...
If your Auth0 tenant does not support email verification (e.g. certain social or enterprise connections), you can set the `BOEING_AUTH0_AUTH_PROVIDER_INSECURE_ALLOW_UNVERIFIED_EMAIL` configuration option to `true` in the Auth Provider configuration dialog. Note that this weakens account and email-domain validation.
:::

You can now return to Boeing and finish configuring Auth0. Use the table below to determine the values to use for each field:

| Boeing          | Auth0         | Auth0 Dashboard Location |
|---------------|---------------|--------------------------|
...
A **Machine to Machine** application is required for Boeing to read user and role information from Auth0.
...
Use the table below to determine the values to use for each field in Boeing's Auth Provider configuration dialog:
...
You can restrict which users can log in to Boeing through Auth0 using connection-level access control or organizations.

##### Using Connection-Level Access Control

You can limit which Auth0 connections (identity providers) are allowed to log in to Boeing.

1. In the Auth0 Dashboard, navigate to **Applications > Applications**
2. Select your Boeing application
3. Open the **Connections** tab
4. Disable any connections that should not be allowed to log in to Boeing
...
5. In **Applications > Applications**, select your Boeing application
...
This section describes the steps involved in switching authentication providers in Boeing. In this example, the provider is switched from **GitHub** to **Microsoft Entra**.
...
Ensure the following environment variable is set in your Boeing installation:

`BOEING_SERVER_FORCE_ENABLE_BOOTSTRAP=true`

### Step 2: Deconfigure the Existing Auth Provider (GitHub)

1. Log in to the admin console (`<boeing-server>/admin`).
...
7. After deconfiguration, you are redirected to the **Welcome to Boeing!** page.

![screenshot of welcome boeing](/img/welcome_boeing_bootstrap.png)

### Step 3: Configure Microsoft Entra as the New Auth Provider

1. On the **Welcome to Boeing!** page, click **Get Started**.
...
1. Log in to the Boeing server (`<boeing-server>`).
...
If a user previously authenticated using GitHub later signs in using Entra with the same email address, Boeing creates a new user record.
