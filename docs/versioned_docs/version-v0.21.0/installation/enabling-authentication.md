...
| `BOEING_BOOTSTRAP_TOKEN` | No | Token used for initial admin login before an auth provider is configured. If not set, a token will be generated and printed to the logs. |
...
- If an auth provider has already been configured, set `BOEING_SERVER_FORCE_ENABLE_BOOTSTRAP=true` to re-enable bootstrap login
