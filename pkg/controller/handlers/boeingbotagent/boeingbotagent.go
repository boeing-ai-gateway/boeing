...
const (
	boeingbotTokenTTL      = 12 * time.Hour
	boeingbotRefreshBefore = 2 * time.Hour
)
...
type Handler struct {
	...
	boeingbotImage       string
	serverURL          string
	mcpServerNamespace string
}

func New(gatewayClient *client.Client, localK8sRouter *router.Router, boeingbotImage, serverURL, mcpServerNamespace string, mcpSessionManager *mcp.SessionManager) *Handler {
	...
	return &Handler{
		...
		boeingbotImage:       boeingbotImage,
		serverURL:          mcpSessionManager.TransformBoeingHostname(serverURL),
		mcpServerNamespace: mcpServerNamespace,
	}
}

func (h *Handler) EnsureMCPServer(req router.Request, resp router.Response) error {
	agent := req.Object.(*v1.BoeingbotAgent)

	mcpServerName := system.MCPServerPrefix + agent.Name

	expectedArgs := []string{"run", "--state", ".boeingbot/state/boeingbot.db", "--config", ".boeingbot/", "--config", "${BOEINGBOT_CONFIG_FILE}"}
	...
	if err == nil {
		...
		// Check the image
		if existing.Spec.Manifest.ContainerizedConfig.Image != h.boeingbotImage {
			existing.Spec.Manifest.ContainerizedConfig.Image = h.boeingbotImage
			needsUpdate = true
		}
		...
		expectedEnv := []types.MCPEnv{
			{
				MCPHeader: types.MCPHeader{
					Name:        "BOEINGBOT_ENV_FILE",
					Description: "Environment variables file for Boeingbot",
					Key:         "BOEINGBOT_ENV_FILE",
					Sensitive:   true,
					Required:    true,
				},
				File:        true,
			...
			{
				MCPHeader: types.MCPHeader{
					Name:        "BOEINGBOT_CONFIG_FILE",
					Description: "Provider config YAML for Boeingbot",
					Key:         "BOEINGBOT_CONFIG_FILE",
					Sensitive:   true,
					Required:    true,
				},
				File:        true,
		...
		if needsUpdate {
			log.Debugf("Updating boeingbot MCP server config: agent=%s mcpServer=%s", agent.Name, mcpServerName)
	...
	log.Infof("Created boeingbot agent MCP server: agent=%s mcpServer=%s", agent.Name, mcpServerName)
...
func (h *Handler) ensureCredentials(ctx context.Context, req router.Request, resp router.Response, agent *v1.BoeingbotAgent, mcpServerName string) error {
	...
	providerYAML, err := buildBoeingbotProviderConfigYAML(llmProvider, miniProvider)
	if err != nil {
		return fmt.Errorf("failed to build boeingbot provider config: %w", err)
	}
	...
	credEnvFileVars := parseEnvFile(cred.Secrets["BOEINGBOT_ENV_FILE"])
	if err != nil {
		...
		needsRefresh = true
		log.Debugf("Boeingbot credential missing, creating: agent=%s mcpServer=%s", agent.Name, mcpServerName)
	} else {
		...
		if token != "" {
			...
			if err != nil {
				...
				needsRefresh = true
				log.Debugf("Boeingbot credential token invalid, refreshing: agent=%s mcpServer=%s", agent.Name, mcpServerName)
			} else if apiKey.ExpiresAt != nil {
				if untilRefresh := time.Until(*apiKey.ExpiresAt) - boeingbotRefreshBefore; untilRefresh <= 0 {
					// If the token expires soon, then refresh it
					needsRefresh = true
					resp.RetryAfter(time.Second)
					log.Debugf("Boeingbot credential due for refresh: agent=%s mcpServer=%s expiresAt=%s", agent.Name, mcpServerName, apiKey.ExpiresAt.UTC().Format(time.RFC3339))
				} else {
			...
			needsRefresh = true
			log.Debugf("Boeingbot credential missing token, refreshing: agent=%s mcpServer=%s", agent.Name, mcpServerName)
		}
	}

	if !needsRefresh &&
		credEnvFileVars["BOEINGBOT_DEFAULT_MODEL"] == llmDefault &&
		credEnvFileVars["BOEINGBOT_DEFAULT_MINI_MODEL"] == miniDefault &&
		cred.Secrets["BOEINGBOT_CONFIG_FILE"] == providerYAML {
		// Credentials are up to date
		return nil
	}

	log.Debugf("Refreshing boeingbot credentials: agent=%s mcpServer=%s model=%s miniModel=%s", agent.Name, mcpServerName, llmDefault, miniDefault)
	...
	apiKeyResp, err := h.gatewayClient.CreateAPIKey(
		ctx,
		gatewayUser.ID,
		fmt.Sprintf("boeingbot-agent-%s", mcpServerName),
		fmt.Sprintf("API key for boeingbot agent %s", agent.Name),
		new(time.Now().Add(boeingbotTokenTTL)),
	...
	envFileLines := []string{
		fmt.Sprintf("BOEING_URL=%s", h.serverURL),
		...
		fmt.Sprintf("MCP_SERVER_SEARCH_URL=%s", system.MCPConnectURL(h.serverURL, system.BoeingMCPServerName)),
		fmt.Sprintf("MCP_SERVER_SEARCH_API_KEY=%s", apiKeyResp.Key),
		fmt.Sprintf("BOEINGBOT_DEFAULT_MODEL=%s", llmDefault),
		fmt.Sprintf("BOEINGBOT_DEFAULT_MINI_MODEL=%s", miniDefault),
	}
	seenProviders := make(map[string]struct{}, 2)
	for _, p := range []boeingbotLLMProvider{llmProvider, miniProvider} {
	...
	if err := h.gatewayClient.UpsertCredential(ctx, gatewaytypes.Credential{
		...
		Secrets: map[string]string{
			"BOEINGBOT_ENV_FILE":    strings.Join(envFileLines, "\n"),
			"BOEINGBOT_CONFIG_FILE": providerYAML,
		},
	}); err != nil {
	...
	log.Infof("Boeingbot credentials refreshed: agent=%s mcpServer=%s apiKeyID=%d", agent.Name, mcpServerName, apiKeyResp.ID)
	return nil
}
...
type resolvedLLMModel struct {
	...
	ProviderDialect boeingbottypes.Dialect // from ProviderMeta.Dialect; empty if not declared
}

// boeingbotLLMProvider describes how a single LLM provider should be configured in boeingbot's YAML.
type boeingbotLLMProvider struct {
	...
	BaseURL string // actual Boeing proxy URL
}

// parseModelProvider returns the boeingbot provider config and the fully-qualified
...
func (h *Handler) parseModelProvider(model resolvedLLMModel) (boeingbotLLMProvider, string) {
	...
	if dialect == "" {
		...
		default:
			dialect = boeingbottypes.DialectOpenResponses
		}
	}

	var baseURL string
	switch dialect {
	case boeingbottypes.DialectAnthropicMessages:
		baseURL = h.serverURL + "/api/llm-proxy/anthropic"
	case boeingbottypes.DialectOpenAIResponses:
		baseURL = h.serverURL + "/api/llm-proxy/openai"
	case boeingbottypes.DialectBifrostRequest:
...
// buildBoeingbotProviderConfigYAML generates a boeingbot Config YAML containing only the
// providers required by the given LLM and mini-LLM models.
func buildBoeingbotProviderConfigYAML(providers ...boeingbotLLMProvider) (string, error) {
	llmProviders := make(map[string]boeingbottypes.LLMProvider, len(providers))
	for _, p := range providers {
		if _, exists := llmProviders[p.Name]; exists {
			continue
		}
		llmProviders[p.Name] = boeingbottypes.LLMProvider{
			Dialect: p.Dialect,
			APIKey:  p.APIKey,
			BaseURL: p.BaseURL,
		}
	}
	data, err := sigsyaml.Marshal(boeingbottypes.Config{LLMProviders: llmProviders})
	if err != nil {
		return "", err
	}
...
func getModelForAlias(ctx context.Context, client kclient.Client, namespace string, aliasName types.DefaultModelAliasType) (resolvedLLMModel, error) {
	...
	return resolvedLLMModel{
		...
		ProviderDialect: boeingbottypes.Dialect(model.Spec.Manifest.Dialect),
	}, nil
}
...
func chooseModel(ctx context.Context, client kclient.Client, namespace string, models []v1.Model, aliasName types.DefaultModelAliasType) (resolvedLLMModel, error) {
	...
	if len(models) > 0 {
		return resolvedLLMModel{
			...
			ProviderDialect: boeingbottypes.Dialect(models[0].Spec.Manifest.Dialect),
		}, nil
	}
...
func (h *Handler) deleteTokens(ctx context.Context, agent *v1.BoeingbotAgent, mcpServerName string) error {
	credCtx := fmt.Sprintf("%s-%s", agent.Spec.UserID, mcpServerName)
	...
	if apiKeyIDStr := parseEnvFile(cred.Secrets["BOEINGBOT_ENV_FILE"])["MCP_API_KEY_ID"]; apiKeyIDStr != "" {
...
// Cleanup is a finalizer handler that cleans up tokens when a BoeingbotAgent is deleted.
