<script lang="ts">
	...
	import type { ChatMessageItemToolCall, ProjectLayoutContext } from '$lib/services/boeingbot/types';
	...
	const chatForSession = $derived(
		$boeingbotChat?.chat?.chatId === sessionId ? ($boeingbotChat?.chat ?? null) : null
	);
	...
	const files = $derived.by(() => {
		...
		const workflowResources = workflowId ? ($boeingbotChat?.resources ?? []) : [];
		const deduped: Array<
