<script lang="ts">
	...
	import {
		...
		isCancellationError,
		parseToolFilePath
	} from '$lib/services/boeingbot/utils';
	...
	function isVisibleWrittenFile(item: ChatMessageItem): boolean {
		...
		return !!filePath && !filePath.includes('/.boeingbot/');
	}
	...
	function isCancelledErrorResource(item: ChatMessageItem): boolean {
		...
		return mime === 'application/vnd.boeingbot.error+json' && isCancellationError(item.resource.text);
	}
	...
	const hasCompactionSummary = $derived(
		viewableMessageItems[0]?._meta?.['ai.boeingbot.meta/compaction-summary']
	);

	const isAttachmentContext = $derived(
		viewableMessageItems[0]?._meta?.['ai.boeingbot.meta/attachment']
	);
</script>
