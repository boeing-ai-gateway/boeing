<script lang="ts">
	import type { Attachment, ChatResult, Prompt, PromptArgument } from '$lib/services/boeingbot/types';
...
{#if !open}
	<div
		class="card bg-base-100 cursor-pointer shadow-md transition-shadow hover:shadow-lg"
		onclick={handleClick}
	...
	>
		<div class="card-body">
			...
			{#if prompt.description}
				<p class="text-base-content/70 text-sm">{prompt.description}</p>
			{/if}
			{#if prompt.arguments && prompt.arguments.length > 0}
