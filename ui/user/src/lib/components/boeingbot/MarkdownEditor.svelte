<script lang="ts">
	import { splitFrontmatter } from '$lib/services/boeingbot/utils';
...
<style>
	...
	:global(.boeingbot .milkdown .ProseMirror) {
		padding-top: 0;
		padding-bottom: 0;
	...
	:global(.boeingbot .milkdown .ProseMirror.block-editor-enabled) {
		padding-left: 5.5rem;
		padding-right: 5.5rem;
	}

	:global(.boeingbot .milkdown .milkdown-code-block) {
		border-radius: var(--radius-box);
	}

	.readonly :global(.boeingbot .milkdown .ProseMirror p.crepe-paragraph:empty::before),
	.readonly :global(.boeingbot .milkdown .ProseMirror [data-placeholder]::before) {
		display: none;
	}

	.readonly
		:global(.boeingbot .milkdown .ProseMirror p:has(> .ProseMirror-trailingBreak:only-child)) {
		display: none;
	}
</style>
