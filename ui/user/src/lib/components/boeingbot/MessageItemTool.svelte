<script lang="ts">
	import MessageItemUI from '$lib/components/boeingbot/MessageItemUI.svelte';
	...
	import { isSafeImageMimeType } from '$lib/services/boeingbot/utils';
	...
	let singleUIResource = $derived(
		item.output?.content &&
			item.output?.content?.filter((i) => {
				return isUIResource(i) && !i.resource?._meta?.['ai.boeingbot.meta/workspace'];
			}).length === 1
	);
	...
	const hasRenderableOutputContent = $derived.by(() => {
		...
		return content.some((contentItem) => {
			if (
				...
				!contentItem.resource._meta?.['ai.boeingbot.meta/workspace']
			)
				return true;
			if (
...
{#if hasRenderableOutputContent}
	<div class="flex w-full flex-wrap items-start justify-start gap-2 p-2">
		{#each item.output!.content ?? [] as contentItem, i (i)}
			{#if contentItem.type === 'resource' && contentItem.resource && isUIResource(contentItem) && !contentItem.resource._meta?.['ai.boeingbot.meta/workspace']}
				<MessageItemUI
					item={contentItem}
					{onSend}
