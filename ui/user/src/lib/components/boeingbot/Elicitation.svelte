<script lang="ts">
	...
	function isOAuthElicitation(): boolean {
		return elicitation.mode === 'url' || Boolean(elicitation._meta?.['ai.boeingbot.meta/oauth-url']);
	}

	function getOAuthUrl(): string {
		return elicitation.url || (elicitation._meta?.['ai.boeingbot.meta/oauth-url'] as string);
	}
	...
	function isQuestionElicitation(): boolean {
		return Boolean(elicitation._meta?.['ai.boeingbot.meta/question']);
	}

	const questions: QuestionData[] = $derived.by(() => {
		const raw = elicitation._meta?.['ai.boeingbot.meta/question'];
		if (!raw) return [];
...
{#snippet elicitationServerHeader(
	...
	{@const serverIcon = elicitationMeta?.['ai.boeingbot.meta/server-icon'] as string}
	{@const serverName = elicitationMeta?.['ai.boeingbot.meta/server-name'] as string}
