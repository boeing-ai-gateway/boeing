<script lang="ts">
	...
	import type { PublishedArtifactVersion } from '$lib/services/boeingbot/types';
	...
	async function getVersionContents(version: number) {
		latestVersionContents = await BoeingbotService.getPublishedArtifactVersionContents(
			publishedArtifactId,
			version
		);
		...
		if (contentsToCompare) {
			...
			const result = await $boeingbotChat?.api.readResource(workflowUri);
