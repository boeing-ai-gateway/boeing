<script lang="ts">
	import type { Chat } from '$lib/services/boeingbot/types';
	import { parseJSON } from '$lib/services/boeingbot/utils';
	...
	let sessionsToShow = $derived.by(() => {
		...
		return sessions.filter(
			(t) => !sessionIdsToFilter.has(t.id) && t.id !== $boeingbotChat?.api?.sessionId
		);
	});
