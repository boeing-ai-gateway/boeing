...
export async function listAllBoeingbotAgents(opts?: { fetch?: Fetcher }): Promise<ProjectV2Agent[]> {
	const response = (await doGet('/boeingbot-agents', opts)) as ItemsResponse<ProjectV2Agent>;
	return response.items ?? [];
}
