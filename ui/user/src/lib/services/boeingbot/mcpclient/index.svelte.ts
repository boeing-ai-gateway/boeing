...
// This is a simple MCP client that works for specifically how boeingbot will reply. It makes
...
export class SimpleClient {
	...
	async #ensureSSEConnection(): Promise<void> {
		...
		// Build SSE URL with session ID as query parameter
		// (EventSource doesn't support custom headers, so we pass session ID via query param)
		const [basePath, existingQuery] = this.#url.split('?');
		// eslint-disable-next-line svelte/prefer-svelte-reactivity
		const queryParams = new URLSearchParams(existingQuery || '');
		...
		this.#sseConnection.onerror = () => {
			// Extract useful information from the EventSource (the error Event itself has no useful details)
			const readyStateMap: Record<number, string> = {
				[EventSource.CONNECTING]: 'CONNECTING',
				[EventSource.OPEN]: 'OPEN',
				[EventSource.CLOSED]: 'CLOSED'
			};
			const readyState = this.#sseConnection
				? readyStateMap[this.#sseConnection.readyState] || String(this.#sseConnection.readyState)
				: 'unknown';
			const url = this.#sseConnection?.url || 'unknown';
			console.error('[SimpleClient] SSE connection error:', { readyState, url });
			// Connection will automatically try to reconnect if readyState is not CLOSED
		};
